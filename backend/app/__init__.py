...
def create_app():
    ...
    with app.app_context():
        try:
            ...
            db.session.execute(text("ALTER TABLE shopkeepers ADD COLUMN IF NOT EXISTS shop_image VARCHAR(255);"))
            db.session.execute(text("ALTER TABLE shopkeepers ADD COLUMN IF NOT EXISTS profile_image VARCHAR(255);"))
            db.session.execute(text("ALTER TABLE users ADD COLUMN IF NOT EXISTS whatsapp_number VARCHAR(20);"))
            db.session.execute(text("ALTER TABLE users ADD COLUMN IF NOT EXISTS profile_image VARCHAR(255);"))
            db.create_all()
            db.session.commit()
